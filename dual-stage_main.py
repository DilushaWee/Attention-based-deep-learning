#paper:A Dual-Stage Attention-Based Recurrent Neural Network
...
class Encoder(nn.Module):
    ...
    def forward(self, input_data):
        ...
        #input_weighted=input_weighted.cuda()
        #hidden=hidden.cuda()
        #cell=cell.cuda()
...
class Encoder2(nn.Module):
    ...
    def __init__(self, input_size: int, hidden_size: int, T: int):
        ...
        self.lstm_layer = nn.LSTM(input_size=input_size, hidden_size=hidden_size, num_layers=1)
        self.attn_linear = nn.Linear(in_features=2 * hidden_size + T - 1, out_features=1)
        self.regr_linear = nn.Linear(in_features=self.input_size, out_features=input_size)
    ...
    def forward(self, input_data):
        # input_data: (batch_size, T - 1, input_size)
        input_weighted = Variable(torch.zeros(input_data.size(0), self.T - 1, int(self.input_size/5))).to(device)
        input_encoded = Variable(torch.zeros(input_data.size(0), self.T - 1, self.hidden_size)).to(device)
        # hidden, cell: initial states with dimension hidden_size
        hidden = init_hidden(input_data, self.hidden_size).to(device)  # 1 * batch_size * hidden_size
        cell = init_hidden(input_data, self.hidden_size).to(device)
        ...
        modified_data = torch.cat(tmp_input, dim=1)        
        ...
        for t in range(self.T - 1):           
            ...
            attn_weights = self.regr_linear(attn_weights)  # (batch_size, input_size) 
...
class Encoder3(nn.Module):
    ...
    def forward(self, input_data):
        ...
        for t in range(self.T - 1):
            ...
            # Eqn. 8: Get attention weights
            x = torch.cos(self.attn_linear(x.view(-1, self.hidden_size * 2 + self.T - 1)))  # (batch_size * input_size) * 1
            # Eqn. 9: Softmax the attention weights
            attn_weights = tf.softmax(x.view(-1, self.input_size), dim=1)  # (batch_size, input_size)
            # Eqn. 10: LSTM
            weighted_input = torch.mul(attn_weights, input_data[:, t, :])  # (batch_size, input_size)
...
class Encoder4(nn.Module):
    ...
    def __init__(self, input_size: int, hidden_size: int, T: int):
        ...
        self.attn_combined = nn.Linear(in_features=2*(input_size), out_features=input_size)
        self.act=nn.ReLU()
    ...
    def forward(self, input_data):
        ...
        for t in range(self.T - 1):
            # Eqn. 8: concatenate the hidden states with each predictor
            x_input = torch.cat((hidden.repeat(self.input_size, 1, 1).permute(1, 0, 2),
                           cell.repeat(self.input_size, 1, 1).permute(1, 0, 2),
                           input_data.permute(0, 2, 1)), dim=2)  # batch_size * input_size * (2*hidden_size + T - 1)
            #linear weights
            # Eqn. 8: Get attention weights
            x = self.attn_linear(x_input.view(-1, self.hidden_size * 2 + self.T - 1))  # (batch_size * input_size) * 1
            # Eqn. 9: Softmax the attention weights
            #attn_weights_linear = tf.softmax(x.view(-1, self.input_size), dim=1)  # (batch_size, input_size)
            attn_weights_linear = x.view(-1, self.input_size)  # (batch_size, input_size)
            attn_weights_linear=self.act(attn_weights_linear)
            
            #cosine weights
            # Eqn. 8: Get attention weights
            y = self.attn_cos(x_input.view(-1, self.hidden_size * 2 + self.T - 1))  # (batch_size * input_size) * 1
            # Eqn. 9: Softmax the attention weights
            #attn_weights_cos = tf.softmax(y.view(-1, self.input_size), dim=1)  # (batch_size, input_size)
            attn_weights_cos = y.view(-1, self.input_size)  # (batch_size, input_size)
            attn_weights_cos=self.act(attn_weights_cos)

            #combine the two sets of weights
            xy = self.attn_combined(torch.cat((attn_weights_linear,attn_weights_cos), dim=1))  # (batch_size, input_size)
            attn_weights = tf.softmax(xy.view(-1, self.input_size), dim=1)  # (batch_size, input_size)
...
def da_rnn(train_length,train_data: TrainData, n_targs: int, encoder_hidden_size=64, decoder_hidden_size=64,
    ...
    # train_cfg = TrainConfig(T, int(train_data.feats.shape[0] * 0.5), batch_size, nn.MSELoss())
...
def train_iteration(t_net: DaRnnNet, loss_func: typing.Callable, X, y_history, y_target):
    ...
    # print(numpy_to_tvar(X),".......")
    # print(input_weighted)
    # print(input_weighted[0][1][0])
    # if len(input_weighted)>0:
    #  list_weight_1.append((input_weighted[0][0][0]).data)


    #print(input_encoded.is_cuda)
    y_pred = t_net.decoder(input_encoded, numpy_to_tvar(y_history))

    y_true = numpy_to_tvar(y_target)
    # print(len(y_pred),"pred")
    # print(len(y_true),"true")
...
raw_data_1 = pd.read_csv("2011-2012-Sydney.csv", usecols=["Total_consumption","Index","Air_temperature","Relative_humidity"], nrows=100 if debug else None)
raw_data_2 = pd.read_csv("2012-2013-Sydney.csv", usecols=["Total_consumption","Index","Air_temperature","Relative_humidity"], nrows=100 if debug else None)
# raw_data = pd.read_csv(os.path.join("data", "nasdaq100_padding.csv"), nrows=100 if debug else None)
# raw_data_1=raw_data_1.head(1000)
# raw_data_2=raw_data_2.head(1000)
...
da_rnn_kwargs = {"batch_size": 128, "T":48}
...
# y_true_inverse = scaler.inverse_transform(data.targs[config.train_size:])
# y_predicted_inverse = scaler.inverse_transform(final_y_pred)
df_new=pd.DataFrame()
...
# list_true=list_true*(np.std(raw_data_1["Total_consumption"]))+np.mean(raw_data_1["Total_consumption"])
...
# final_y_pred=final_y_pred*(np.std(raw_data_1))+np.mean(raw_data_1)
...
# df_xx=pd.DataFrame()
# df_xx["weights"]=list_weight_1
# df_xx.to_csv("weights.csv")